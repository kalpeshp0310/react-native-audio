...
public class LibFlac {
   ...
   */
  public void initialize(int sampleRate, int channel, int bps, int compressLevel)
  ...
  public void encode(byte[] buffer, int size) {
    if (!this.encode(cPointer, channel, buffer, size)) {
      throw new IllegalArgumentException(
          "buffer encode byte array must be smaller or equal 1024 byte");
    }
  }
  ...
  /**
   * convert raw wave pcm into flac file. Input file must be a raw pcm, a wave raw data with header
   * format.
   ...
   */
  private native int setMetadata(long cPointer, int sampleRate, int channel, int bps,
      int compressLevel);
}
